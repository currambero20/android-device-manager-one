...
public class MDMActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if ("screenshot".equals(action)) { requestScreenshot(); } else { finish(); }
    }
    private void requestScreenshot() {
        MediaProjectionManager manager = (MediaProjectionManager) getSystemService(MEDIA_PROJECTION_SERVICE);
        if (manager != null) { startActivityForResult(manager.createScreenCaptureIntent(), REQUEST_SCREENSHOT); } else { finish(); }
    }
    @Override
