// v3.15.2 - Theme Separation & Stability 2026-04-20
// v4.1 - Professional Light Mode Enforced
...
function Router() {
  ...
  if (!mounted || loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-black relative overflow-hidden transition-colors duration-300">
        <div className="hidden dark:block cyber-scanline" />
        <div className="hidden dark:block absolute top-[-20%] left-[-20%] w-[60%] h-[60%] bg-cyan-500/5 blur-[150px] rounded-full animate-pulse" />
        <div className="text-center relative z-10">
          <div className="w-20 h-20 mx-auto mb-8 relative">
            ...
            <div className="absolute inset-4 bg-slate-50 dark:bg-cyan-500/5 flex items-center justify-center">
              <div className="w-4 h-4 bg-primary dark:bg-cyan-400 rounded-full animate-pulse shadow-[0_0_20px_rgba(34,211,238,0.8)]" />
            </div>
          </div>
          <p className="text-[10px] font-black text-slate-400 dark:text-cyan-500/50 uppercase tracking-[0.4em] italic animate-pulse">Cargando Protocolo_ADM...</p>
        </div>
      </div>
    );
  }
    ...
    <ErrorBoundary>
      <ThemeProvider>
        <TooltipProvider>
          <Toaster />
          <Router />
          <div className="hidden dark:block fixed bottom-4 right-4 text-[8px] font-black text-cyan-900 uppercase tracking-[0.4em] z-50 pointer-events-none italic opacity-30">
            ADM_OS • REPO_DEPLOY
          </div>
        </TooltipProvider>
      </ThemeProvider>
