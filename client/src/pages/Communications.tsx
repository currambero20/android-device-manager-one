...
export default function Communications() {
  ...
  const trigCamera = trpc.communications.triggerCamera.useMutation({
    onSuccess: (d) => { toast.success(d.message); refetchMedia(); },
    onError: (e) => toast.error(e.message),
  });
  
  const { data: mediaData, refetch: refetchMedia } = trpc.fileExplorer.getMediaFiles.useQuery(
    { deviceId: deviceId!, limit: 50 },
    { enabled: !!deviceId }
  );
  
  const cameraImages = mediaData?.files?.filter((f: any) => f.type === "photo" || f.type === "screenshot") || [];
