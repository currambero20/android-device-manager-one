...
export default function DeviceMonitoring() {
  ...
  useEffect(() => {
    const currentIds = devices.map((d: any) => d.id).sort().join(",");
    if (devices.length > 0) {
      if (lastSyncedIdsRef.current !== currentIds) {
        lastSyncedIdsRef.current = currentIds;
        syncDevices(devices.map((d: any) => d.id));
      }
    } else if (lastSyncedIdsRef.current !== "empty") {
      lastSyncedIdsRef.current = "empty";
      clearAll();
    }
  ...
  const currentStatus = selectedDeviceId ? getStatus(selectedDeviceId) : undefined;
      ...
      <div className="space-y-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-cyan-500/10">
          <div>
            <h1 className="text-4xl font-black tracking-tighter uppercase gradient-text flex items-center gap-3">
              <Activity className="w-10 h-10 text-primary dark:text-cyan-400" />
              Centro de Telemétrica
            </h1>
            <p className="text-slate-500 dark:text-cyan-500/60 text-[10px] font-black uppercase tracking-[0.3em] mt-1 italic">Vigilancia de Nodos en Tiempo Real</p>
          </div>
          
          <div className={`flex items-center gap-4 px-6 py-3 rounded-2xl border transition-all ${
            isConnected 
              ? "bg-slate-100 dark:bg-cyan-500/10 border-slate-200 dark:border-cyan-500/40 text-primary dark:text-cyan-400 shadow-sm dark:shadow-[0_0_20px_rgba(34,211,238,0.2)]" 
              : "bg-rose-500/10 border-rose-500/40 text-rose-500"
          }`}>
            <div className={`w-3 h-3 rounded-full ${isConnected ? "bg-primary dark:bg-cyan-400 shadow-sm dark:shadow-[0_0_10px_rgba(34,211,238,1)] animate-pulse" : "bg-rose-500"}`} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            <div className="glass-panel border-slate-200 dark:border-cyan-500/20 shadow-2xl p-6 flex flex-col h-[750px]">
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-black text-slate-900 dark:text-cyan-100 flex items-center gap-3 uppercase text-xs tracking-widest italic">
                  <Smartphone className="w-5 h-5 text-primary dark:text-cyan-500" />
                  Nodos_Activos ({devices.length})
                </h3>
                <Button variant="ghost" size="icon" className="h-10 w-10 text-slate-500 dark:text-cyan-700 hover:text-primary dark:hover:text-cyan-400 hover:bg-slate-100 dark:hover:bg-cyan-500/10 rounded-xl border border-slate-200 dark:border-cyan-500/5">
              ...
              <ScrollArea className="flex-1 pr-2 cyber-scrollbar">
                <div className="space-y-3">
                  {devicesLoading ? (
                    <div className="flex flex-col items-center justify-center py-20 text-slate-300 dark:text-cyan-500/20">
                        ...
                        <div className="relative z-10">
                          ...
                          <div className="flex items-center gap-5 text-[10px] font-black text-cyan-700">
                            <div className="flex items-center gap-2">
                              <Battery className={`w-4 h-4 ${(getStatus(device.id)?.batteryLevel ?? device.batteryLevel) < 20 ? "text-rose-500" : "text-cyan-400"}`} />
                              <span>{getStatus(device.id)?.batteryLevel ?? device.batteryLevel ?? '--'}%</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <Signal className="w-4 h-4 text-fuchsia-500" />
                              <span className="font-mono tracking-tighter">{getSignalBars(getStatus(device.id)?.signalStrength ?? device.signalStrength)}</span>
                            </div>
                          </div>
                        </div>
                        {selectedDeviceId === device.id && (
            ...
            {selectedDevice ? (
              <>
                ...
                <div className="glass-panel border-cyan-500/20 shadow-2xl p-8 relative overflow-hidden group">
                   ...
                   <div className="flex flex-wrap items-center justify-between gap-6 relative z-10 mb-8 border-b border-cyan-500/10 pb-6">
                    ...
                    <div className="flex gap-3">
                      ...
                      <Button 
                        className="btn-neon h-12 px-6 rounded-xl font-black text-[10px] uppercase tracking-widest" 
                        onClick={() => {
                          refreshStatusMutation.mutate({ deviceId: selectedDeviceId! });
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6 relative z-10">
                    {[
                      { label: "Batería", value: (currentStatus?.batteryLevel ?? selectedDevice.batteryLevel) ? `${currentStatus?.batteryLevel ?? selectedDevice.batteryLevel}%` : 'SINC...', icon: Battery, color: "text-cyan-400", bg: "bg-cyan-500/10" },
                      { label: "Signal_Link", value: getSignalBars(currentStatus?.signalStrength ?? selectedDevice.signalStrength), icon: Signal, color: "text-fuchsia-500", bg: "bg-fuchsia-500/10" },
                      { label: "Node_Status", value: (currentStatus?.status ?? selectedDevice.status) === "online" ? "ACTIVE" : "IDLE", icon: Activity, color: "text-emerald-400", bg: "bg-emerald-500/10" },
                      { label: "Last_Heartbeat", value: (currentStatus?.timestamp || selectedDevice.lastSeen) ? formatDistanceToNow(new Date(currentStatus?.timestamp || selectedDevice.lastSeen), { addSuffix: true, locale: es }).toUpperCase() : "SYNCING...", icon: Clock, color: "text-amber-400", bg: "bg-amber-500/10" },
                    ].map((stat, i) => (
                ...
                <Tabs defaultValue="gps" className="w-full">
                  <TabsList className="bg-slate-100 dark:bg-black/40 border border-slate-200 dark:border-cyan-500/20 p-1.5 h-16 rounded-2xl gap-3 mb-8">
                    <TabsTrigger value="gps" className="flex-1 rounded-xl data-[state=active]:bg-white dark:data-[state=active]:bg-cyan-500/20 data-[state=active]:text-primary dark:data-[state=active]:text-cyan-300 data-[state=active]:shadow-sm dark:data-[state=active]:neon-border-cyan font-black text-xs uppercase tracking-widest transition-all">
                      <MapPin className="w-5 h-5 mr-3" />
                      Geolocalización
                    </TabsTrigger>
                    <TabsTrigger value="sms" className="flex-1 rounded-xl data-[state=active]:bg-white dark:data-[state=active]:bg-fuchsia-500/20 data-[state=active]:text-primary dark:data-[state=active]:text-fuchsia-300 data-[state=active]:shadow-sm dark:data-[state=active]:neon-border-magenta font-black text-xs uppercase tracking-widest transition-all">
                  ...
                  {/* GPS View */}
                  <TabsContent value="gps" className="glass-panel border-slate-200 dark:border-cyan-500/10 shadow-2xl p-8 mt-0 animate-in fade-in duration-500">
                    {currentLocation ? (
                      <div className="space-y-8">
                        <div className="bg-slate-50 dark:bg-black/40 p-6 rounded-2xl border border-slate-200 dark:border-cyan-500/10 grid grid-cols-2 md:grid-cols-3 gap-8">
                          <div>
                            <p className="text-[9px] font-black text-slate-400 dark:text-cyan-500 uppercase tracking-[0.3em] mb-2">Latitude_Coord</p>
                            <p className="text-base font-mono font-bold text-slate-900 dark:text-cyan-100">{currentLocation.latitude.toFixed(6)}</p>
                          </div>
                          <div>
                            <p className="text-[9px] font-black text-slate-400 dark:text-cyan-500 uppercase tracking-[0.3em] mb-2">Longitude_Coord</p>
                            <p className="text-base font-mono font-bold text-slate-900 dark:text-cyan-100">{currentLocation.longitude.toFixed(6)}</p>
                          </div>
                          <div>
                            <p className="text-[9px] font-black text-slate-400 dark:text-cyan-500 uppercase tracking-[0.3em] mb-2">Accuracy_Radius</p>
                            <div className="flex items-center gap-3 font-black text-primary dark:text-cyan-400 text-base">
                              <div className="w-2.5 h-2.5 rounded-full bg-primary dark:bg-cyan-500 shadow-sm dark:shadow-[0_0_10px_rgba(34,211,238,1)]" />
                        ...
                        <div className="flex items-center justify-between">
                          <h4 className="font-black text-slate-900 dark:text-cyan-100 flex items-center gap-3 uppercase text-xs tracking-widest italic">
                             <MapIcon className="w-5 h-5 text-primary dark:text-cyan-500" />
                          ...
                          <Button 
                            ...
                            className="text-primary dark:text-cyan-500 hover:text-white dark:hover:text-cyan-300 hover:bg-primary dark:hover:bg-cyan-500/10 font-black text-[9px] uppercase tracking-widest rounded-xl border border-slate-200 dark:border-cyan-500/20 px-4 transition-all"
                          >
                        ...
                        {showMap && currentLocation && (
                          <div className="space-y-6">
                            <div className="w-full h-[450px] rounded-3xl overflow-hidden border-2 border-slate-200 dark:border-cyan-500/30 shadow-sm dark:shadow-[0_0_40px_rgba(34,211,238,0.1)] dark:grayscale">
                              <MapView
                      ...
                      <div className="text-center py-32 bg-slate-50 dark:bg-cyan-950/20 rounded-3xl border-2 border-dashed border-slate-200 dark:border-cyan-500/10">
                        <MapPin className="w-20 h-20 text-slate-200 dark:text-cyan-900/20 mx-auto mb-6" />
                        <h4 className="text-xl font-black text-slate-800 dark:text-cyan-100 uppercase tracking-widest italic">Signal_Lost</h4>
                        <p className="text-[10px] text-slate-400 dark:text-cyan-900 font-bold uppercase tracking-[0.3em] max-w-xs mx-auto mt-6">Sin coordenadas de telemetría recientes para este nodo.</p>
                      </div>
                    )}
                  </TabsContent>
