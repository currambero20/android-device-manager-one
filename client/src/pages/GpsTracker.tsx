...
export default function GpsTracker() {
  ...
  const refreshGpsMutation = trpc.geofencing.requestRefresh.useMutation({
    onSuccess: () => {
      toast.success("Solicitud de ubicación enviada al dispositivo");
      setTimeout(() => refetchLocation(), 2000);
    },
    onError: (e) => toast.error(e.message),
  });
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => refreshGpsMutation.mutate({ deviceId: selectedDeviceId })}
                disabled={!selectedDeviceId || refreshGpsMutation.isPending}
                className="border-emerald-200 text-emerald-700 hover:bg-emerald-50"
              >
                <RefreshCw className={`w-4 h-4 mr-2 ${refreshGpsMutation.isPending ? "animate-spin" : ""}`} />
                {refreshGpsMutation.isPending ? "Solicitando..." : "Actualizar"}
              </Button>
            </div>
          </CardContent>
        </Card>
