...
export default function MediaCapture({ deviceId }: { deviceId?: number | null }) {
  ...
  const { data: mediaData, refetch, isLoading } = trpc.fileExplorer.getMediaFiles.useQuery(
    ...
    <DashboardLayout title="Captura Remota">
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          {/* Controls Panel */}
          <Card className="lg:col-span-1 border-slate-200 dark:border-accent/20 shadow-xl bg-white dark:bg-slate-900 text-slate-900 dark:text-white overflow-hidden">
            <CardHeader className="border-b border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5">
            ...
            <CardContent className="space-y-6 pt-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex p-1 bg-slate-50 dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/10">
                  <button
                    onClick={() => setSelectedCamera("back")}
                    className={`flex-1 py-2 text-xs rounded-md transition-all ${selectedCamera === "back" ? "bg-rose-600 text-white shadow-lg" : "text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}
                  >
                    Principal
                  </button>
                  <button
                    onClick={() => setSelectedCamera("front")}
                    className={`flex-1 py-2 text-xs rounded-md transition-all ${selectedCamera === "front" ? "bg-rose-600 text-white shadow-lg" : "text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}
                  >
                    Frontal
                  </button>
                </div>
              ...
              <div className="space-y-3 pt-4 border-t border-white/5">
                ...
                <Button 
                  ...
                  className={`w-full ${isRecordingAudio ? "bg-rose-600 animate-pulse text-white" : "bg-slate-100 dark:bg-white/10 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white hover:bg-slate-200 dark:hover:bg-white/20" }`}
                ...
                <Button 
                  ...
                  className={`w-full ${isRecordingVideo ? "bg-rose-600 animate-pulse text-white" : "bg-slate-100 dark:bg-white/10 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white hover:bg-slate-200 dark:hover:bg-white/20" }`}
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Input
                      ...
                      min={5}
                      max={600}
                      className="w-20 h-7 bg-slate-50 dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-900 dark:text-white text-xs text-center"
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Select value={screenQuality} onValueChange={(v: any) => setScreenQuality(v)}>
                      <SelectTrigger className="w-20 h-7 bg-slate-50 dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-900 dark:text-white text-xs">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
        ...
        {/* Security Warning */}
        <div className="bg-slate-100 dark:bg-slate-900 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 text-[10px] leading-relaxed">
          <p className="font-bold text-slate-900 dark:text-slate-200 mb-1 flex items-center gap-2 uppercase tracking-widest">
