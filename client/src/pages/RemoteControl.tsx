  ...
  return (
    <DashboardLayout title="Control Remoto">
        ...
        <Card className="card-neon overflow-hidden lg:col-span-1">
          <div className="p-4 border-b border-glow-cyan/20 flex justify-between items-center">
            <h2 className="font-bold flex items-center gap-2">
              <Smartphone className="w-4 h-4 text-primary dark:text-cyan-400" />
              Dispositivos
            </h2>
              ...
              <button
                ...
                className={`w-full p-3 rounded-lg border transition-all text-left ${
                  selectedDeviceId === device.id
                    ? "border-primary dark:border-cyan-500 bg-slate-100 dark:bg-cyan-500/10 shadow-sm dark:shadow-[0_0_10px_rgba(6,182,212,0.3)]"
                    : "border-slate-200 dark:border-border hover:border-primary dark:hover:border-cyan-500/50 hover:bg-slate-50 dark:hover:bg-accent/50"
                }`}
              >
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="card-neon-magenta overflow-hidden">
            <div className="p-4 border-b border-glow-magenta/20">
              <h2 className="font-bold flex items-center gap-2">
                <Zap className="w-4 h-4 text-primary dark:text-magenta-400" />
                Acciones de Control
              </h2>
            </div>
            <div className="p-6">
              {!selectedDeviceId ? (
                ...
                <Tabs defaultValue="control">
                  <TabsList className="grid w-full grid-cols-2 mb-6 bg-slate-100 dark:bg-accent/20">
                    <TabsTrigger value="control" className="data-[state=active]:bg-white dark:data-[state=active]:bg-background">Comandos</TabsTrigger>
                    <TabsTrigger value="history" className="data-[state=active]:bg-white dark:data-[state=active]:bg-background">Historial Real</TabsTrigger>
                  </TabsList>

                  <div className="mb-4 flex justify-end">
                      <Button variant="link" size="sm" className="text-magenta-400 font-bold p-0" asChild>
                      <Link href={`/media-capture/${selectedDeviceId}`} className="text-primary dark:text-cyan-400 hover:text-primary/80 dark:hover:text-cyan-300 font-bold flex items-center gap-1">
                    ...
                    {commandTypes.map((cmd: any) => (
                      <Button
                        ...
                        className={`h-20 flex-col gap-2 text-[10px] font-bold uppercase transition-all ${
                          cmd.dangerous ? "hover:bg-red-600 hover:border-red-400" : "hover:border-primary dark:hover:border-cyan-500 hover:text-primary dark:hover:text-cyan-400"
                        }`}
                      >
                        {cmd.label}
                  ...
                  <TabsContent value="history" className="space-y-2">
                    ...
                    {commands.length === 0 ? (
                      ...
                      commands.map((cmd: any) => (
                        <div key={cmd.id} className="p-2 bg-slate-50 dark:bg-accent/5 rounded border border-slate-200 dark:border-border flex items-center justify-between">
      ...
      <AlertDialog open={showConfirmation} onOpenChange={setShowConfirmation}>
        <AlertDialogContent className="border border-red-500 bg-white dark:bg-black/95">
          <AlertDialogTitle className="text-red-500 flex items-center gap-2">
            <AlertTriangle className="w-5 h-5" />
            Acción Crítica
          </AlertDialogTitle>
          <AlertDialogDescription className="text-slate-500 dark:text-muted-foreground">
            Estás a punto de enviar un comando de alto riesgo: <span className="text-primary dark:text-white font-bold underline">{selectedCommand}</span>. 
