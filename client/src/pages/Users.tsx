...
export default function Users() {
  ...
  return (
    <DashboardLayout title="Usuarios">
      <div className="hidden dark:block cyber-scanline" />
      <div className="space-y-8 relative z-10">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 pb-6 border-b border-slate-200 dark:border-primary/10">
          <div>
            <h1 className="text-4xl font-black tracking-tighter uppercase text-slate-900 dark:gradient-text flex items-center gap-3">
              <UsersIcon className="w-10 h-10 text-primary dark:animate-pulse" />
              Directorio de Personal
            </h1>
            <p className="text-slate-500 dark:text-primary/40 text-[10px] font-black uppercase tracking-[0.3em] mt-1 italic">Control de Accesos y Privilegios MDM</p>
          </div>
          <div className="flex items-center gap-4 w-full md:w-auto">
            <div className="relative flex-1 md:w-80">
              ...
              <input
                ...
                className="w-full h-12 pl-10 pr-4 bg-white dark:bg-black/40 border border-slate-200 dark:border-primary/20 rounded-xl text-sm font-medium focus:outline-none focus:border-primary transition-all"
              />
            </div>
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <Button className="btn-sapsan h-12 px-6" onClick={() => { setEditingUserId(null); setFormData({ name: "", email: "", password: "", role: "user" }); setIsDialogOpen(true); }}>
                  <Plus className="w-5 h-5 mr-3" /> Registrar
                </Button>
              </DialogTrigger>
              <DialogContent className="glass-panel bg-white dark:bg-black/95 border-slate-200 dark:border-primary/30 text-foreground max-w-md">
                <DialogHeader className="mb-6">
                  ...
                  <DialogDescription className="text-muted-foreground font-bold uppercase text-[10px] tracking-widest">
                    Protocolo de autenticación nivel 1
                  </DialogDescription>
                </DialogHeader>
                <form onSubmit={handleCreateOrUpdate} className="space-y-5">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-primary tracking-widest ml-1">Nombre Completo</label>
                    <Input placeholder="DESIGNACIÓN" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="input-sapsan h-12" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-primary tracking-widest ml-1">Dirección Mail</label>
                    <Input placeholder="EMAIL_IDENTITY" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} type="email" className="input-sapsan h-12" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-primary tracking-widest ml-1">Clave de Acceso</label>
                    <Input placeholder="ENCRYPTED_KEY" type="password" onChange={e => setFormData({...formData, password: e.target.value})} className="input-sapsan h-12" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-primary tracking-widest ml-1">Rango / Rol</label>
                    <Select value={formData.role} onValueChange={v => setFormData({...formData, role: v})}>
                      <SelectTrigger className="input-sapsan h-12 font-bold uppercase tracking-widest">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className="bg-white dark:bg-black/95 border-primary/30">
                        <SelectItem value="admin">ADMIN_PRIME</SelectItem>
                        <SelectItem value="manager">SECTOR_MANAGER</SelectItem>
                        <SelectItem value="user">FIELD_AGENT</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  <Button type="submit" className="w-full btn-sapsan h-12 font-black tracking-widest mt-6" disabled={createUserMutation.isPending}>
                    {createUserMutation.isPending ? "PROCESANDO..." : "CONFIRMAR REGISTRO"}
                  </Button>
                </form>
              </DialogContent>
            </Dialog>
        ...
        <div className="glass-panel overflow-hidden border-slate-200 dark:border-cyan-500/20 shadow-sm dark:shadow-[0_0_40px_rgba(0,0,0,0.5)] bg-white dark:bg-black/60">
          <Table>
            <TableHeader className="bg-slate-50 dark:bg-cyan-500/5">
              <TableRow className="border-b border-slate-200 dark:border-cyan-500/10 h-14">
