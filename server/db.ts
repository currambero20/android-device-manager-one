...
export async function createUser(user: { name: string; email: string; role: string; passwordHash?: string }): Promise<UserRow | undefined> {
  ...
  // Primero validamos si el correo ya existe
  const existing = await getUserByEmail(user.email);
  if (existing) {
    throw new Error(`Ya existe un usuario registrado con el correo: ${user.email}`);
  }
  ...
  try {
    ...
    throw new Error("No se pudo crear el usuario. Por favor intenta otro correo.");
  }
