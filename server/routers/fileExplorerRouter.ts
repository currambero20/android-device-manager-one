  ...
  getDirectoryContents: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      try {
        ...
        const deviceResult = await db.select().from(devices).where(eq(devices.id, input.deviceId)).limit(1);
  ...
  getMediaFiles: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      try {
        ...
        // Obtener archivos con sintaxis correcta de Drizzle ORM
        const files = await db
          .select()
          .from(mediaFiles)
          .where(and(...whereConditions))
          .orderBy(mediaFiles.timestamp)
          .limit(input.limit)
          .offset(input.offset);
