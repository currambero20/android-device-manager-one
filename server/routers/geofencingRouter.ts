...
export const geofencingRouter = router({
  ...
  /**
   * Solicitar actualización inmediata de GPS al dispositivo
   */
  requestRefresh: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const wsManager = getWebSocketManager();
      if (wsManager) {
        console.log(`[WebSocket] Requesting real-time GPS update for device ${input.deviceId}`);
        wsManager.broadcastToDevice(input.deviceId, "get-location", {
          payload: {}
        });
        return { success: true };
      }
